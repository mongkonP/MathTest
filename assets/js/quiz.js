...
class GeometryQuiz {
    ...
    newQuestions() {
        ...
        this.container.innerHTML = this.questions.map((question, index) => this.createQuestion(question, index)).join("");

        this.container.querySelectorAll("[data-check-answer]").forEach((button) => {
            button.addEventListener("click", () => this.checkAnswer(Number(button.dataset.checkAnswer)));
        });
    }
    ...
    createQuestion(question, index) {
        ...
        const hint = question.hint ? `<small class="quiz-hint">${question.hint}</small>` : "";
    ...
    checkAnswer(index) {
        ...
        const isCorrect = input.value.trim() !== "" && Number.isFinite(answer) && Math.abs(answer - correct) <= tolerance;
